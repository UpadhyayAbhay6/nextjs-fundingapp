    ...
    return (
        <nav className='bg-gray-900 shadow-lg shadow-white text-white flex justify-between px-4 items-center h-16'>
            <Link href={"/"} className='logo font-bold text-lg flex justify-center items-center gap-2'>
                <Image className='rounded-full' width={44} src="./https://cdn3.vectorstock.com/i/1000x1000/45/82/funding-platform-project-financing-icon-vector-38754582.jpg" alt="" />
                <span>FundingExpress!</span>
            </Link>
            <div className='relative'>
                {session && <><button onClick={()=>setShowdropdown(!showdropdown)} onBlur={()=>{setTimeout(()=>{
                    setShowdropdown(false)
                },100);}} id="dropdownDefaultButton" data-dropdown-toggle="dropdown" className="text-white mx-4 
                ...
                dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">Welcome {session.user.name}
                ...
                viewBox="0 0 10 6">
                    ...
                    <div id="dropdown" className={`z-10 ${showdropdown?"":"hidden"} left-[125px] absolute bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`}>
                        <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                            ...
                            <li>
                                <Link href={`/${session.user.name}`} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" onClick={console.log(session.user.name)}>Your Page</Link>
                            </li>
                ...
                {session && <Link href={"http://localhost:3000/login"}><button type="button" className="text-white bg-gradient-to-br from-purple-600
